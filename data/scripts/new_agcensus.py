...
def farmsize(df):
    ...
    df = df.drop(df[(df.livestock=='poultry') &
                    (df.size_min==1) &
...
def get_counts():
    ...
    # cattle
    print('--------------------------------------------------\ncattle')
    dfs = df[(df.livestock=='CATTLE') & (df.statisticcat_desc=='INVENTORY')].copy()
    ...
    dfs = df[(df.group_desc=='POULTRY') &
             (df.livestock!='CHICKENS')].copy()
    dfs.loc[dfs.livestock=='POULTRY, OTHER', 'livestock'] = 'OTHER'
    dfs = dfs.drop(dfs[(dfs.livestock=='OTHER') &
    ...
    # chickens
    print('--------------------------------------------------\npoultry except chicken')
    dfs = df[df.livestock=='CHICKENS'].copy()
    dfs.subtype = dfs.class_desc
    dfs.subtype = dfs.subtype.map({'ROOSTERS': 'ckn-roosters', 
