...
print('Compute score for each state.')
score = df.groupby(PARLIST + ['state_code'])[['kst', 'ksst']].sum().reset_index()
score['score'] = score.kst / score.ksst
## .apply(
...
score = score.drop(['_merge'], axis=1)

# prepare vectors for clustering (using the state scores
# fv(\underline(\alpha))
print('Dump after formatting.')
