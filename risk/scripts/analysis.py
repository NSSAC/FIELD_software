...
def performance(cr):
    # download datasets
    fname = f'../results/risk_scores_sp1_cr{cr}.csv.zip'
    rs = pd.read_csv(fname)
    __, events, __, __ = risk.load_features()
    outprefix = re.sub('.csv.zip', '', os.path.basename(fname))

    # set time range
    poultry_range = pd.date_range(start='2024-01', end='2025-12', freq="MS").strftime('%Y-%m').tolist()
    dairy_range = pd.date_range(start='2025-04', end='2025-12', freq="MS").strftime('%Y-%m').tolist()

    tdf = rs[(rs.subtype!='milk') & (rs.time.isin(poultry_range))]
    pdf_ = pd.melt(tdf, id_vars=['county_code', 'subtype', 'time'],
                  var_name='ahead', value_name='risk')
    tdf = rs[(rs.subtype=='milk') & (rs.time.isin(dairy_range))]
    ddf_ = pd.melt(tdf, id_vars=['county_code', 'subtype', 'time'],
                  var_name='ahead', value_name='risk')
    ddf_['subtype'] = 'milk'

    pdf_.ahead = pdf_.ahead.astype('int')
    ddf_.ahead = ddf_.ahead.astype('int')

    # choose only those counties that have farms.
    # for spillover, these are also the counties with non-zero risk
    # select relevant counties that actually have farms (according to our data and thresholds)
    # get farms
    dfarms = utils.load_dairy_farms(aggregate='county', commercial=True,
                                     commercial_threshold=100)
    dfarms = utils.fit_central_valley(dfarms)
    ...
    pdf = pdf_.merge(farms, on=['county_code', 'subtype'])
    ddf = ddf_.merge(farms, on=['county_code', 'subtype'])
    
    # merge with ground truth 
    events['ym'] = events.start_date.dt.strftime('%Y-%m')
    events = events.rename(columns={'type': 'subtype'})
    events = events.drop('start_date', axis=1).drop_duplicates()
    ...
    # find rows in events whose county_code is absent in risk
    missing_instances = events.merge(
        pd.concat([pdf, ddf]), left_on=['county_code', 'subtype', 'ym'],
        right_on=['county_code', 'subtype', 'time'], 
        how='left', indicator=True)
    missing_instances = missing_instances[missing_instances['_merge'] == 'left_only']
    missing_instances = missing_instances[['county_code', 'subtype', 'ym']].drop_duplicates()

    # Integrate with risk all these counties that are missing farms
    ### assign [1,2,3,4,5,6] to column "ahead" and explode
    missing_instances['ahead'] = [list(range(6))] * len(missing_instances)
    missing_instances = missing_instances.explode('ahead')
    missing_instances['time'] = missing_instances.apply(
        lambda row: pd.to_datetime(row['ym']) - pd.DateOffset(
            months=row['ahead']), axis=1) 
    missing_instances['time'] = missing_instances['time'].dt.strftime('%Y-%m')

    # get risk for missing instances
    missing_instances.ahead = missing_instances.ahead.astype('int')
    missing_instances['state_code'] = missing_instances.county_code // 1000

    pmissing = missing_instances[missing_instances.subtype!='milk'].merge(
        pdf_[['county_code', 'subtype', 'time', 'ahead', 'risk']], 
        on=['county_code', 'subtype', 'time', 'ahead'], how='left').fillna(0)
    dmissing = missing_instances[missing_instances.subtype=='milk'].merge(
        ddf_[['county_code', 'subtype', 'time', 'ahead', 'risk']], 
        on=['county_code', 'subtype', 'time', 'ahead'], how='left').fillna(0)

    pdf = pd.concat([pdf, pmissing], ignore_index=True)
    ddf = pd.concat([ddf, dmissing], ignore_index=True)

    # rank across the time ranges per subtype
    labels = ['Very low', 'Low', 'Medium', 'High', 'Very high']
    markers = {'Very low': 'o', 'Low': 's', 'Medium': 'D', 'High': '^', 'Very high': 'X'}
    percentiles = [0, 25, 50, 75, 95, 100]
    #percentiles = [0, 50, 75, 90, 100]
    prap = pdf.groupby(['subtype', 'ahead'])[['county_code', 'time', 'ahead', 'risk']].apply(
            utils.percentile_categorize, timecol='time', riskcol='risk',
            start_time=poultry_range[0], end_time=poultry_range[-1],
            exclude_zeros=False, percentiles=percentiles, labels=labels)
    drap = ddf.groupby(['subtype', 'ahead'])[['county_code', 'time', 'ahead', 'risk']].apply(
            utils.percentile_categorize, timecol='time', riskcol='risk',
            start_time=dairy_range[0], end_time=dairy_range[-1],
            exclude_zeros=False, percentiles=percentiles, labels=labels)
    rap = pd.concat([prap, drap], ignore_index=False)
    rap = rap.drop('ahead', axis=1).reset_index() # ahead is in the index
    ...
    rap.ahead = rap.ahead.astype('int') + 1

    # finalize rap
    rap = rap.drop('level_2', axis=1)

    # merge events and risk
    res = events.merge(rap, left_on=['county_code', 'subtype', 'ym'],
                       right_on=['county_code', 'subtype', 'ym_ahead'], how='left')
    if res.isnull().sum().sum():
        raise ValueError('Found some unexpected NaNs in rap.')
    ...
    for subtype in subtypes:
        ...
        counts = tdf.groupby(['ahead', 'rank_per_period'], observed=True).size().reset_index(
                name='count')
        ...
        for i,val in enumerate(profiles):
            if val in hue_values:
                ...
                current_markers[val] = markers[val]
        ...
        pe = plot.Lineplot(subplot=sp, data=counts, x='ahead', y='percent',
                           ...
                           markers=current_markers, markersize=10, palette=current_palette)
        ...
        sp.title(value=fr'{subtype_title} \#regions={tdf.county_code.drop_duplicates().shape[0]}/{rap[rap.subtype==subtype].county_code.drop_duplicates().shape[0]}')
        pe.legend()
        fig.savefig(f'eval_{outprefix}_{subtype}.pdf')
    ...
    res = res.merge(farms, on=['county_code', 'subtype'], how='left')
    
    # select only counties with farms if absent
    res = res[~((res.present=='absent') & (res.farms.isnull()))]

    # this is for later, should be moved down
    ...
    for subtype in subtypes:
        ...
        if subtype == 'milk':
            ...
            subtype_title = subtype
        sp.title(value=fr'{subtype_title} \#regions={tdf.county_code.drop_duplicates().shape[0]}')
        pe.legend()
    ...
    for subtype in subtypes:
        ...
        if subtype == 'milk':
            ...
            subtype_title = subtype
        sp.title(value=fr'{subtype_title} \#counties={tdf.county_code.drop_duplicates().shape[0]}')
        pe.legend()
        fig.savefig(f'values-wo-outliers_{outprefix}_{subtype}.pdf')

    # False positives
    dairy_range = pd.date_range(start='2025-04', end='2025-10', freq="MS").strftime('%Y-%m').tolist()
    print('Generating false positives plots ...')
    tdf = res[['state', 'county_code', 'subtype', 'ahead', 'time', 
               'rank_per_period', 'present']].drop_duplicates()
    ...
    cpers = tdf[['state', 'county_code', 'time', 'subtype', 'ahead', 
                 'rank_per_period']].drop_duplicates().drop(
                      'time', axis=1).value_counts().reset_index()
    cdf = cpres.merge(cpers, on=['state', 'county_code', 'subtype', 'ahead'])
    ### Merge with farms
    cdf = cdf.merge(farms, on=['county_code', 'subtype'], how='left')

    if cdf.isnull().sum().sum():
        print('Found some NaNs in the merge of county farms. Removing those rows.')
        print(cdf.isnull().sum())
    cdf = cdf[~cdf.isnull().any(axis=1)]
    ...
    spres = tdf.groupby(['state', 'subtype', 'ahead'], as_index=False)[
             'present'].max()
    spers = tdf[['state', 'time', 'subtype', 'ahead', 'rank_per_period']
             ].drop_duplicates().drop('time', axis=1).value_counts().reset_index()
    sdf = spres.merge(spers, on=['state', 'subtype', 'ahead'])
    sdf = sdf.merge(sfarms, on=['state', 'subtype'], how='left')
    if sdf.isnull().sum().sum():
        print('Found some NaNs in the merge of state farms. Removing those rows.')
        print(sdf.isnull().sum())
    sdf = sdf[~sdf.isnull().any(axis=1)]
    ...
    for subtype in subtypes:
        ...
        if subtype == 'milk':
            ...
            rng = poultry_range
        states_present = events[(events.subtype==subtype) & (events.ym<=rng[-1])].state.drop_duplicates().tolist()
        tdf = cdf[(cdf.subtype==subtype) & (cdf.ahead==1) &
                  (~cdf.state.isin(states_present)) &
                  (cdf.rank_per_period=='Very high') &
        ...
        fp_poly = plot.Polyplot(subplot=sp, data=fp_states,
                                facecolor='#dddddd', label='No reports')
        farm_threshold = 5
        if gcdf.shape[0] == 0:
            print('Encountered zeros')
            continue
...
def select_county_risk():
    ...
    # load both risks
    spdf = pd.read_csv('../results/risk_scores_sp1_cr0.csv.zip')
    spdf = spdf[['county_code', 'time', 'subtype', '0']].rename(columns={'0': 'risk'})
    spdf['type'] = 'WB spillover'

    totdf = pd.read_csv('../results/risk_scores_sp1_cr1.csv.zip')
    ...
    for subtype in ['milk', 'turkeys']:
        ...
        for county_name, county_code in select_counties[subtype].items():
            ...
            sp.ylabel(value='Inc. prob.', width_ratio=.7)
            if subtype == 'milk':
                subtype_title = 'dairy cattle'
            else:
                subtype_title = subtype
            ...
            vals, labs = sp.get_ticks('y')
            # choose alternate labels and values
            if len(vals) > 6:
                vals = vals[::2]
                labs = labs[::2]

                # add one more tick if vals is even
                if len(vals) % 2 == 0:
                    ymax = vals[-1] + (vals[1]-vals[0])
                    vals.append(ymax)
                    labs.append(rf'$\mathdefault{{{ymax}}}')

            pe = plot.Scatterplot(subplot=sp, data=events_county, 
                                 x='ym', y=[vals[-1]*.5]*events_county.shape[0],
                                 s=100, color=3, label='Outbreaks reported')
            sp.yticks(ticks=vals, labels=labs)
...
def state_risk(model):
    if model == 'spillover':
        risk_file = '../results/risk_scores_sp1_cr0.csv.zip'
    elif model == 'comprehensive':
        risk_file = '../results/risk_scores_sp1_cr1.csv.zip'
