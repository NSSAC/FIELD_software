...
from aaviz import plot_new as plot
...
def outbreaks(delta, hop):
    ...
    ax = plot.oneplot(fg_x=5, fg_y=4, 
                      ...
                      pf_hue='type', pf_hue_order=types,
                      pf_markers=markers,
                      pf_style='type', pf_markersize=10,
                      sp_xscale='log',
...
def performance(cr):
    ...
    ##     # Count the occurrences per x ('total_bill') and hue ('sex')
    ##     tdf = res[res.subtype==subtype]
    ##     counts = tdf.groupby(['ahead', 'rank_across_periods']).size().reset_index(
    ##             name='count')
    ...
    ##     markers = ['o', '^', 's', 'o', 'x']
    ##     hue_order = []
    ##     hue_values = counts.rank_across_periods.drop_duplicates().tolist()
    ##     for val in ['Very high', 'High', 'Medium', 'Low', 'Very low']:
    ##         if val in hue_values:
    ##             hue_order.append(val)
    ##     ax = plot.oneplot(fg_x=4, fg_y=3, 
    ##                       func='sns.lineplot', data=counts, 
    ##                       pf_x='ahead', pf_y='percent', pf_hue='rank_across_periods', 
    ##                       pf_style='rank_across_periods', pf_markersize=10,
    ##                       pf_style_order=hue_order,
    ##                       pf_markers=markers,
    ##                       pf_hue_order=hue_order,
    ##                       sp_xlim=(1,counts.ahead.max()), sp_ylim=(0,100),
    ##                       la_xlabel=r'$k$-step ahead forecast', 
    ##                       la_ylabel=r'\% instances', la_title=subtype, lg_title='',
    ##                       fs_xlabel='large', fs_ylabel='large')
    ...
    ##                       la_xlabel=r'$k$-step ahead forecast', 
    ##                       la_ylabel=r'Risk score $R(c,t)$', la_title=subtype, lg_title='',
    ##                       fs_xlabel='Large', fs_ylabel='Large')
    ##     plot.savefig(f'values_{outprefix}_{subtype}.png', dpi=200)
    ##     plt.clf()
    ## # without fliers
    ## for subtype in subtypes:
    ##     ax = plot.oneplot(fg_x=4, fg_y=3, 
    ...
    ##                       fs_xlabel='Large', fs_ylabel='Large')
    ##     ## tdf = res[res.subtype==subtype]
    ##     ## tdf = tdf.groupby(['ahead', 'present'], group_keys=False
    ##     ##                   ).apply(utils.remove_outliers, column='risk')
    ##     ## ax = plot.oneplot(fg_x=5, fg_y=4, 
    ##     ##                   func='sns.violinplot', 
    ##     ##                   data=tdf, 
    ##     ##                   pf_x='ahead', pf_y='risk', pf_hue='present', 
    ##     ##                   pf_inner='quart', pf_split=True, pf_gap=.1,
    ##     ##                   pf_cut=0,
    ##     ##                   la_xlabel=r'$k$-step ahead forecast', 
...
def ww_h5():
    ...
    # ww negatives: given negatives, are there correponding outbreak?
    ### load ww monitoring
    ww = utils.wastewater()

    ### select those which have never been positive
    ww.groupby('county_code').agg({'time': 'min', 'present': 'max'})
    wodf = ww.groupby('county_code').agg({'time': 'min', 'present': 'max'}).reset_index()
    ...
    ### Plot
    fig = plot.Fig(x=15, y=8, constrained_layout=True)
    fig.grid(nrows=2, ncols=3, hspace=-1, wspace=-1)
    i = 0
    for subtype in ['dairy', 'turkeys', 'ckn-layers', 'mixed', 'backyard', 'ckn-broilers']:
        tdf = nwwh5[nwwh5.subtype==subtype].groupby('county_code_ww').agg(
                {'h5_instances': 'sum', 'length': 'min'}).reset_index()
        neg_counties = tdf.shape[0]
        ## if subtype == 'dairy':
        ##     tdf = tdf[tdf.county_code_ww//1000!=6]
        cdf = counties.merge(tdf, left_on='county_code', right_on='county_code_ww')
        sp = plot.Subplot(fig=fig, row=i//3, col=i%3, projection='gcrs.WebMercator()')
        poly = plot.Polyplot(subplot=sp, data=states)
        chor = plot.Choropleth(subplot=sp, data=cdf, hue='h5_instances', cmap='cbYlOrRd')
        sp.legend(plot_element=chor, shrink=.5, labelsize='footnotesize', pad=0, 
                  location='bottom', title=r'\#H5 reports', 
                  title_fontsize='footnotesize') # colorbar arguments
    ...
    tdf = wwh5[['event_ww', 'event_h5']].fillna(-1).groupby('event_ww').max()
    if not tdf.shape[0]:
        raise ValueError(
                'Found a ww incident that is no where near a h5 in space or time! Not really an error, but soomething has changed.')
    ...
    pwwh5 = pwwh5[['county_code_ww', 'subtype', 'h5_instances', 'ww_instances', 
                   'length', 'dist']].drop_duplicates()
    ...
    i = 0
    for subtype in ['dairy', 'turkeys', 'ckn-layers', 'mixed', 'backyard', 'ckn-broilers']:
        tdf = pwwh5[pwwh5.subtype==subtype].groupby('county_code_ww').agg(
                {'h5_instances': 'sum', 'ww_instances': 'first', 'length': 'min'})
        ...
        sp.legend(plot_element=noreports, handlelength=1)
        print(subtype, tdf.h5_instances.min(), tdf.h5_instances.max())
        i += 1
    ...
    sp = plot.Subplot(fig=fig, xlim=(0,500), ylim=(0,100))
    ...
    for subtype in subtypes:
        print(subtype)
        tdf = nwwh5[nwwh5.subtype==subtype].copy()
        nunique = pd.Series({
            dist: tdf[tdf.dist<=dist]['county_code_ww'].nunique()
            for dist in tdf.dist.unique()}, name='hits').reset_index().rename(
                    columns={'index': 'dist'})
    ...
    fig = plot.Fig(x=5, y=4)
    sp = plot.Subplot(fig=fig, xlim=(0,10), ylim=(0,100))
...
def state_risk():
    ...
    df.to_csv('state_' + os.path.basename(risk_file), index=False)
