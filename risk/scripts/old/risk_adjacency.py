...
def baseline_spillover_model():
    ...
    for type in ['milk', 'turkeys', 'ckn-layers', 'ckn-broilers', 'ducks', 
        ...
        ##             ]
        ## else:
        search_space = [Real(0.000001, 1, prior='log-uniform', name="birds_W")]
        bounds = [(0,1)]
...
def baseline_conditional_risk_model():
    pdf = utils.h5n1_poultry_with_county_neighbors(commercial=True)

    ddf = utils.h5n1_dairy_with_county_neighbors()
    ddf['type'] = 'milk'

    reps = pd.concat([pdf, ddf]).reset_index(drop=True)
    ...
    for t in reps.type.drop_duplicates():
        if t in ['waterfowl', 'ckn-pullets']:
            continue
        if t == 'milk':
            months = ['2023-01', '2025-04']
        else:
            months = ['2022-01', '2023-12']
        ## if t != 'milk':
        ##     continue
...
def _type_baseline_conditional_risk(df, dates):
    ...
    sequences = df.pivot_table(index="county_code", columns="time", 
                              values="report", aggfunc="max").fillna(0)
    ...
    for params in ParameterGrid(param_grid):
        ...
        for random_state in [1, 2, 3, 4, 5]:
            ...
            model = hmm.CategoricalHMM(**params, n_features=4)  # 4 categorical states
...
def susceptibility_score(period, bird_h5_prevalence=False,
    ...
    # load data
    farms, __, outbreaks, h5_prevalence = load_features()
    farms = process_features(farms)

    pdf = utils.h5n1_poultry_with_county_neighbors(commercial=True)
    ddf = utils.h5n1_dairy_with_county_neighbors()
    ddf['type'] = 'milk'
    events = pd.concat([pdf, ddf]).reset_index(drop=True)
    ...
    events['ym'] = events.year.astype('str') + '-' + events.month.astype('str').str.zfill(2)
...
def _type_total_risk(events, dates, spm, crm, numsims=NUMSIMS, cr_flag=True):
    ...
    # AA: create event matrix including county adjacency
    events = events.rename(columns={'report': 'present'})
    ...
    edf = edf.reset_index().merge(spdf[['county_code']], 
                                  on='county_code',
                                  how='right').fillna(0)
        ...
        if cr_flag:
            ...
            for i in range(pseqs.shape[0]):
                ...
                for s in range(numsims):
                    ...
                    fseqs_list.append(np.mod(observations.flatten(),2))
                    sseqs_list.append(__states)

                if pseq[-1] == 1 or pseq[-1] == 3:
                    set_trace()
...
# AA
def risk_score(susceptibility_scores, alpha_list):
    # read farms
    pf = utils.load_poultry_farms(commercial=True)
    df = utils.load_dairy_farms(commercial=True)
    df = utils.fit_central_valley(df)
    # just get number of farms
    farms = pd.concat([pf, df]).reset_index(drop=True).groupby(
            ['county_code', 'subtype'], as_index=False)['fid'].count().rename(
                columns={'fid': 'num_farms'})
    rdf = susceptibility_scores.merge(farms, on=['county_code', 'subtype'], how='left')
    rdf['num_farms'] = rdf['num_farms'].fillna(0)
    risk_list = []
    for alpha in alpha_list:
        tdf = rdf.copy()
        for k in range(6):
            tdf[k] = tdf[k] * tdf.num_farms ** alpha
        tdf['alpha'] = alpha
        risk_list.append(tdf)
    risk_scores =  pd.concat(risk_list)
    return risk_scores
...
if __name__ == '__main__':
    ...
    if args.mode == 'spillover_model':
        ...
        susceptibility_df = susceptibility_score([args.period_start, args.period_end], 
        ...
        susceptibility_df.to_csv(
                f'susceptibility_sp{int(args.spillover_risk_model)}_cr{int(args.conditional_risk_model)}.csv.zip', 
                index=False)
        
        risk_df = risk_score(susceptibility_df, [0.1, 0.5, 1])
        risk_df.to_csv(
