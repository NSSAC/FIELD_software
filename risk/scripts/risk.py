...
# if we have no reports for the past x months, we do not compute conditional risk
CONDITIONAL_RISK_BUFFER = 6
...
def load_features():
    # This is generated in utils.py. It contains all COMMERCIAL farms with
    # associated wild bird population.
    ...
    for params in ParameterGrid(param_grid):
        ...
        for random_state in [1, 2, 3, 4, 5]:
            ...
            train_seqs, val_seqs = train_test_split(
                    sequences, test_size=0.2, random_state=random_state)
            X_train, lengths_train = prepare_data(train_seqs)
            X_val, lengths_val = prepare_data(val_seqs)

            model = hmm.CategoricalHMM(**params, n_features=2)  # 2 categorical states
            ...
            score_list.append(model.aic(X_val, lengths_val))
            model_list.append(model)

        score = np.min(score_list)
        ...
        if score < best_score:
            ...
            best_params['random_state'] = np.argmin(score_list)+1

    # train model on all data for the best params
    best_model = hmm.CategoricalHMM(**best_params, n_features=2)
    X_train, lengths_train = prepare_data(sequences)
    best_model.fit(X_train, lengths_train)  # Train only on training sequences
...
def _type_total_risk(events, dates, spm, crm, numsims=NUMSIMS, cr_flag=True):
    ...
    # prepare data for spillover risk
    ...
    # 1-k months forecast for each time step
    risk_per_date = []
    for t in dates:
        print('\t', t)
        ...
        # get spillover risk for next months
        pred_spdf = spdf[['county_code', 'subtype'] + next_months].copy()
        pred_spdf.columns = ['county_code', 'subtype'] + list(range(MAX_FORWARD_PRED))
        pred_spdf['time'] = t

        if not cr_flag:
            risk_per_date.append(pred_spdf)
        ...
        ## We don't simulate spillover risk here, as it is already a probability
        ...
        for i in range(pseqs.shape[0]):
            # only choose past CONDITIONAL_RISK_BUFFER months or length of pseqs
            if pseqs.shape[1] > CONDITIONAL_RISK_BUFFER:
                pseq = pseqs[i,-CONDITIONAL_RISK_BUFFER:]
            else:
                pseq = pseqs[i,:]
            ...
            for s in range(numsims):
                ...
                observations, __states = crm.sample(n_samples=MAX_FORWARD_PRED, currstate=initial_state)
                fseqs_list.append(observations.flatten())
                
            crisk_list.append(np.array(fseqs_list))
        cr_sims = np.stack(crisk_list, axis=0)

        # combine spillover and conditional risk
        ## rmat = np.mean(np.logical_or(sp_sims, cr_sims), axis=1)
        crmat = np.mean(cr_sims, axis=1)

        # Now, combine the probabilities
        ## get matrix form of spillover risk
        spmat = np.zeros(crmat.shape)
        for f in range(MAX_FORWARD_PRED):
            spmat[:,f] = pred_spdf[f].values
        # combined risk
        compmat = 1 - (1 - spmat) * (1 - crmat)

        # A basic verification within a rounding error
        if (compmat - spmat < -ROUND_THRESHOLD).any() or (compmat < crmat - ROUND_THRESHOLD).any():
            raise ValueError('Combined risk less than individual risks')
        
        print(f'#non-zero conditional risk entries: {np.sum(crmat>0)}')

        # create standard format for comprehensive risk
        pred_compdf = pd.DataFrame(compmat, columns=range(MAX_FORWARD_PRED))
        pred_compdf[['time','subtype']] = (t, subtype)
        pred_compdf['county_code'] = edf.county_code
        risk_per_date.append(pred_compdf)
...
if __name__ == '__main__':
    ...
    if args.mode == 'spillover_model':
        ...
        risk_scores.to_csv(
                f'risk_scores_sp{int(args.spillover_risk_model)}_cr{int(args.conditional_risk_model)}.csv.zip', 
                index=False)
